...
public final class PlayerDataDao {
    ...
    public PlayerSettings getSettings(UUID uuid) throws SQLException {
        String sql = "SELECT allow_invites, show_warnings, show_tag, ff_mode FROM player_data WHERE uuid = ?";
        try (PreparedStatement statement = databaseManager.getConnection().prepareStatement(sql)) {
            statement.setString(1, uuid.toString());
            try (ResultSet resultSet = statement.executeQuery()) {
                if (!resultSet.next()) {
                    return new PlayerSettings(true, true, true, FfMode.AUTO);
                }
                FfMode ffMode;
                try {
